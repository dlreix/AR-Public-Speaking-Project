...
public class AudienceSpawner : MonoBehaviour
{
    ...
    public void SpawnAudience()
    {
        ...
        // If we are in the meeting room, we want all seats occupied regardless of stress level
        if (VRSpeakingTrainer.Core.AppManager.Instance != null && 
            VRSpeakingTrainer.Core.AppManager.Instance.SelectedEnvironment == EnvironmentType.meeting_room)
        {
            maxToSpawn = allSeats.Count;
        }
        ...
        for (int i = 0; i < maxToSpawn; i++)
        {
            ...
            // ── 1) Instantiate ──
            // HEURISTIC: Our procedural generators (Classroom/Conference) create seats facing "backwards" (Z+ is back).
            // External models (Round tables/Chairs) usually have Z+ as "front".
            float yOffset = 0f;
            string seatName = seat.name.ToLower();
            if (seatName.StartsWith("seat") || seatName.StartsWith("bench"))
            {
                yOffset = 180f;
            }
            
            Quaternion faceForward = Quaternion.Euler(0, seat.transform.eulerAngles.y + yOffset, 0);
    ...
    private List<GameObject> FindAllSeats()
    {
        ...
        foreach (GameObject go in allObjects)
        {
            ...
            // Discovery: Classroom (Seat_*), Conference (Bench_*), Meeting (Chair/MeetingSeat)
            if (n.Contains("seat") || n.Contains("bench") || n.Contains("chair") || n.Contains("meeting"))
            {
                // Exclude structural components (legs, sides, backs, frames)
                if (!n.Contains("leg") && !n.Contains("side") && !n.Contains("back")
                    && !n.Contains("frame") && !n.Contains("body") && !n.Contains("arm")
                    && !n.Contains("base") && !n.Contains("table") && !n.Contains("group")) // Don't treat the table or groups as a seat!
                {
