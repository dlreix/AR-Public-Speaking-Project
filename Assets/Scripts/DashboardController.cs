...
{
    [Header("Overview Page (Genel Bakış)")]
    ...
    [Header("Performance Page (Yeni Detaylı Sütunlar)")]
    ...
    [Header("History Page (Geçmiş ve Grafik)")]
    ...
    {
        Debug.Log("[DashboardController] Veri çekme (Import) işlemi başladı...");

        if (DataManager.Instance == null)
        {
            Debug.LogError("[DashboardController] HATA: DataManager bulunamadı!");
            return;
        }

        if (!AppRuntimeState.HasInstance)
        {
            Debug.LogError("[DashboardController] HATA: AppRuntimeState bulunamadı! Motor veriyi tutmuyor.");
            return;
        }
        ...
        {
            Debug.LogWarning("[DashboardController] AppRuntimeState bulundu ama oturum verisi BOŞ (null).");
            return;
        }
        ...
        if (basarili)
        {
            Debug.Log("<color=green>[DashboardController] MÜKEMMEL! Sahne geçişinde konuşma verisi başarıyla yakalandı ve kaydedildi!</color>");
        }
        else
        {
            Debug.LogWarning("[DashboardController] Veri bulundu ancak DataManager kaydetmeyi reddetti (veri 0 veya aynı veri az önce kaydedildi).");
        }
    }
    ...
    public void ResetSystem()
    {
        if (DataManager.Instance != null)
        {
            ...
            Debug.Log("Sistem Sıfırlandı! Grafik temizlendi ve etiketler s1-s9 yapıldı.");
        }
    }
}
