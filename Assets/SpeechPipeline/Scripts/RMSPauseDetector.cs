...
{
    // Classifies audio chunks as voiced or silent using RMS energy.
    // Fires OnPauseDetected when continuous silence exceeds PauseThreshold.
    public sealed class RMSPauseDetector
    {
        // 0.015 suits a typical quiet room; raise if background noise is high.
        public float NoiseFloor     = 0.015f;
        public float PauseThreshold = 1.5f;
        // Prevents brief unvoiced bursts from flickering IsSpeaking.
        public float Hysteresis     = 0.08f;

        // Fires when silence first exceeds PauseThreshold. Arg = silence duration so far.
        ...
        // Call on resume so stale silence does not trigger a spurious OnPauseDetected.
        public void Reset()
        {
            IsSpeaking   = false;
            IsInPause    = false;
